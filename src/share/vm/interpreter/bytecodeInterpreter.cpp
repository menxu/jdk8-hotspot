...
#define ARRAY_STOREFROM64(T, T2, stackSrc, extra)                                    \
      ...
      CASE(_monitorenter): {
        ...
        //// lock record(lr) -> BasicObjectLock
        ...
        if (entry != NULL) {
          ...
          if (mark->has_bias_pattern()) {
            ...
            ////当前线程ID
            thread_ident = (uintptr_t)istate->thread();
            // code 4: 计算判断当前线程ID是否等于对象头里面的线程ID
            anticipated_bias_locking_value =
              (((uintptr_t)lockee->klass()->prototype_header() | thread_ident) ^ (uintptr_t)mark) &
              ~((uintptr_t) markOopDesc::age_mask_in_place);
          ...
          if (!success) {
            ...
            ////首选产生一个无锁的mark word 00
            markOop displaced = lockee->mark()->set_unlocked();
            ////把锁记录中的mark改为无锁的
            entry->lock()->set_displaced_header(displaced);
            //如果指定了-XX:+UseHeavyMonitors，则call_vm=true，代表禁用偏向锁和轻量级锁
            bool call_vm = UseHeavyMonitors;
            ////如果此时对象头也是无锁的，则修改entry的地址 为什么会失败？
